...
##### **Synchronized** 和 CAS的区别 <https://zhuanlan.zhihu.com/p/134085332> 

```
1: CAS:
	CAS（Compare And Swap ）是乐观锁的一种实现方式，是一种轻量级锁。JAVA1.5开始引入了CAS，JUC下很多工具类都是基于CAS.
	CAS带来的问题: ABA问题， 通过版本号可以解决
				 循环开销过大： CAS操作不成功的话，会导致一直自旋，CPU的压力会很大。例如说Unsafe下的getAndAddInt方法会一直循环，知道成功才会返回。
				 
2: synchronized
	相比于CAS基于乐观锁实现，synchronized是基于悲观锁的，当一个线程试图访问同步代码块时，它首先必须得到锁，退出或抛出异常时必须释放锁。
	2.1.1： 对于普通同步方法加锁时，锁是当前实例对象
	2.2.2： 对于静态同步方法加锁时，锁是当前类的Class对象
	2.2.3： 对于同步方法块加锁时，锁是Synchonized括号里配置的对象
 	synchronized的优化
 	
 	2.2.1： synchronized锁升级：偏向锁 → 轻量级锁 → 重量级锁     		     			   参考： https://zhuanlan.zhihu.com/p/71156910
 	
 	2.3.1：一次只能允许一个线程进入被锁住的代码块
    2.3.2： synchronized是一种内置锁/监视器锁
    2.3.3： Java中每个对象都有一个内置锁(监视器,也可以理解成锁标记)，而synchronized就是使用对象的内置锁(监视器)来将代码块(方法)锁定的！
```
...
##### synchronized的原理 <https://juejin.im/post/6844903670933356551#heading-1> 